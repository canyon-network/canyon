...
mod tests {
    ...
    use canyon_primitives::{Block, DigestItem, Signature};
    use canyon_runtime::constants::{currency::CENTS, time::SLOT_DURATION};
    use canyon_runtime::{Address, BalancesCall, Call, UncheckedExtrinsic};
    use codec::Encode;
    ...
    #[test]
    // It is "ignored", but the canyon-cli ignored tests are running on the CI.
    // This can be run locally with `cargo test --release -p canyon-cli test_sync -- --ignored`.
    #[ignore]
    fn test_sync() {
